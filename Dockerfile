...
# 使用仓库内的 s6-overlay（文件名已确认）
COPY assets/s6-overlay-aarch64.tar.xz /tmp/s6-overlay-aarch64.tar.xz

# 正确解压 .tar.xz（一次性解压）
RUN tar --use-compress-program=xz -xf /tmp/s6-overlay-aarch64.tar.xz -C / && \
